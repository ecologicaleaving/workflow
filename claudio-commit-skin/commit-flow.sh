...
# Claudio Commit Skin - Pre-Commit Hook
# Automatically updates PROJECT.md, builds project, and stages artifacts

set -e

SKIN_DIR=".commit-skin"
LOG_FILE="${SKIN_DIR}/commit.log"

# Create skin directory if missing
mkdir -p "${SKIN_DIR}"
...
log "🔧 Claudio Commit Skin activated"

# Load project configuration
if [ ! -f "${SKIN_DIR}/project-config.json" ]; then
    log "❌ Project configuration missing. Run install.sh first."
    exit 1
fi

PROJECT_NAME=$(jq -r '.project_name' "${SKIN_DIR}/project-config.json")
PROJECT_TYPE=$(jq -r '.project_type' "${SKIN_DIR}/project-config.json")
GITHUB_URL=$(jq -r '.github_url' "${SKIN_DIR}/project-config.json")

log "📁 Project: ${PROJECT_NAME} (${PROJECT_TYPE})"

# Check if PROJECT.md exists
if [ ! -f "PROJECT.md" ]; then
    log "❌ PROJECT.md not found. Creating from template..."
...
log "🔄 Version increment (${COMMIT_TYPE}): v${CURRENT_VERSION} → v${NEW_VERSION}"
...
log "   ✅ PROJECT.md updated with v${NEW_VERSION}"
...
case "${PROJECT_TYPE}" in
    "flutter")
        ...
        if [ -f "android/app/build.gradle" ]; then
            log "   Building APK..."
            if flutter build apk --release > "${SKIN_DIR}/build.log" 2>&1; then
                ...
                if [ -f "${APK_PATH}" ]; then
                    ...
                    log "   ✅ APK built: ${RELEASE_APK}"
                    BUILD_SUCCESS=true
                fi
            else
                log "   ⚠️  APK build failed, check ${SKIN_DIR}/build.log"
            fi
        fi
    ...
    "nodejs")
        ...
        if [ -f "package.json" ] && grep -q '"build"' package.json; then
            ...
            if npm run build > "${SKIN_DIR}/build.log" 2>&1; then
                if [ -d "build" ] || [ -d "dist" ]; then
                    ...
                    ARTIFACTS+=("${RELEASE_WEB}")
                    log "   ✅ Web build packaged: ${RELEASE_WEB}"
                    BUILD_SUCCESS=true
                fi
            else
                log "   ⚠️  Build failed, check ${SKIN_DIR}/build.log"
            fi
        fi
        ;;
    ...
    "static")
        ...
        log "   ✅ Static site packaged: ${RELEASE_STATIC}"
        BUILD_SUCCESS=true
        ;;
...
git add PROJECT.md
log "   ✅ PROJECT.md staged"

# Stage any built artifacts
for artifact in "${ARTIFACTS[@]}"; do
    git add "${artifact}"
    log "   ✅ Staged artifact: ${artifact}"
done

# Create a build summary comment for the commit message  
if [ ${#ARTIFACTS[@]} -gt 0 ]; then
    SUMMARY_FILE="${SKIN_DIR}/commit-summary.txt"
    
    cat > "${SUMMARY_FILE}" << EOF

Auto-generated by claudio-commit-skin:
- Updated PROJECT.md: v${CURRENT_VERSION} → v${NEW_VERSION}
...
EOF
    ...
    log "   ✅ Commit message enhanced with build summary"
fi

# Final status
if [ "${BUILD_SUCCESS}" = true ]; then
    log "🎉 Commit preparation complete!"
    log "   Version: v${NEW_VERSION}"
    log "   Artifacts: ${#ARTIFACTS[@]}"
    log "   Ready for push to GitHub"
else
    log "⚠️  Commit prepared but build had issues"
    log "   Check logs in ${SKIN_DIR}/"
fi

log "🔧 Claudio Commit Skin finished"
exit 0