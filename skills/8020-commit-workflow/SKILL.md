...
- If commit skin is installed, it handles this automatically
...
After all checks pass, stage and commit:
```bash
git add <specific-files>          # Never use git add -A blindly
git commit -m "type: description"
```

If commit skin is active, it will automatically:
...
6. Stage everything and log to `.commit-skin/commit.log`
